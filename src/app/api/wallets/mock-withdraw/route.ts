  ...
  try {
    ...
    const userTransaction = await db
      ...
      .values({
        ...
        currencyType: currencyType,
        status: 'completed',
        description: `Mock withdrawal of ${amount} ${currencyType} (Net: ${netAmount.toFixed(2)}, Commission: ${commissionAmount.toFixed(2)})`,
        referenceId: paymentAddress || paymentMethod || null,
    ...
    const adminTransaction = await db
      ...
      .values({
        ...
        description: `Commission from withdrawal by user ${user.email}`,
        createdAt: new Date().toISOString()
      })
      .returning();
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        paymentAddress: paymentAddress || 'Mock Address',
